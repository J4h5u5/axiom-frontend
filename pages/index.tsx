...
export const getServerSideProps: GetServerSideProps = async () => {
  let res;
  try {
    res = await fetcher<{ status: string; data: { usersCount: number } }>(
      `${API_URL}/usersCount`
    );
  } catch (error) {
    res = 0;
          ...
          <section className="wrapper">
            ...
            <div className="interactive-wrapper">
              <div className="form">
                ...
                <div className="auth-wrapper" id="authWrapper">
                  <button className="form__btn bg-goldenDream text-text">
                    ПОЕХАЛИ!
                  </button>
                  <button className="form__btn_hover text-text">
                    Регистрация в ТГ
                  </button>
            ...
            <div className="description">
              <span className="description__text text-text">
                Вся необходимая информация будет доступна в{' '}
                <span>Зале Ожидания</span> после <span>регистрации</span> и
                получения <span>посадочного талона</span>.
              </span>
            </div>
